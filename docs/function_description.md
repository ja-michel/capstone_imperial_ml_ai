Description of the eight synthetic black-box functions you’ll be optimising. Each one is designed to mimic real-world complexity, with features such as non-linearity and multiple local maxima. 

You’ll only be able to query each function occasionally, so every choice counts. [Initial data](../data/initial_data) Links to an external site.is provided as `.npy` files; use `np.load()` in Python to examine them. 
...
### <a name="f3"></a>Function 3
- **Inputs**: 3d array (15,3)
- **Ouput**: 1d array (15,)
- **Goal**: Maximise
